...
public class ReservaMesaController {
    ...
    public ResponseEntity<?> createReservaMesa(@RequestBody ReservaMesaDTO dto) {
        try {
            ...
            BigDecimal precioTotal = mesa.getPrecioBase();
            ...
            reservaMesa.setTurno(dto.getTurno());
            reservaMesa.setHora(dto.getHora());
            reservaMesa.setMontoPago(precioTotal);
            reservaMesa.setNumeroPersonas(dto.getCapacidadPersonas());
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'CLIENT')")
    public ResponseEntity<List<HorasOcupadas>> getHorasOcupadas(@PathVariable Long id) {

        List<ReservaMesa> reservaMesaList = reservaMesaService.findAll();
        List<HorasOcupadas> horasOcupadasList = new ArrayList<>();

        for (ReservaMesa reservaMesa : reservaMesaList) {
            if (reservaMesa.getMesa().getId().equals(id)) {
                HorasOcupadas hora = new HorasOcupadas(reservaMesa.getFecha(), reservaMesa.getHora());
                horasOcupadasList.add(hora);
            }
        }

        return ResponseEntity.ok(horasOcupadasList);
    }
}
