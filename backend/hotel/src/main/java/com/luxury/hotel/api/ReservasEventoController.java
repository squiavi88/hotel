...
public class ReservasEventoController {
    ...
    public ReservasEventoController(ReservaEventoService reservaEventoService, ReservaEventoRepository reservaEventoRepository, ReservaService reservaService, EventoService eventoService) {
    ...
    @GetMapping("/reservas-eventos/ocupadas/{fecha}")
    @PreAuthorize("hasAnyRole('ADMIN', 'CLIENT')")
    public ResponseEntity<List<ReservaEvento>> getActividadesOcupadas(@PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fecha) {

        List<ReservaEvento> ocupadasList = reservaEventoRepository.findByFecha(fecha);
